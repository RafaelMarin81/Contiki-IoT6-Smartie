...
init_net(void)
{
  uip_ipaddr_t ip_um_net;
  uip_ip6addr(&ip_um_net, 0x2001, 0x720, 0x1710, 0x2, 0x123, 0, 0, 0);
  ...
  uip_ds6_prefix_add(&ip_um_net, UIP_DEFAULT_PREFIX_LEN, 0);
#endif /* UIP_CONF_ROUTER */
  uip_ds6_set_addr_iid(&ipaddr, &uip_lladdr);
  uip_ds6_addr_add(&ipaddr, 0, ADDR_TENTATIVE);
  
  uip_ds6_set_addr_iid(&ip_um_net, &uip_lladdr);
  uip_ds6_addr_add(&ip_um_net, 0, ADDR_TENTATIVE);
