...
static option_type manage_response(char *response_buf, coap_packet_t *request){	
	...
	if (strcmp(request->uri_path,"/.well-known/core")==0) {
		...
		return CONTENT;
	}else if(strcmp(request->uri_path,"/sensors/mdl")==0){		
		PRINTF("Request Query to: /sensors/mdl \n");
		if(request->code == GET){
			sprintf(response_buf, "Clitech Mote 6LoWPAN-JN5149");
			return CONTENT;
		}
		return METHOD_NOT_ALLOWED;
	}else if(strcmp(request->uri_path,"/sensors/ip")==0){
		PRINTF("Request Query to: /sensors/ip \n");
		if (request->code==GET) {			
			sprintf(response_buf, "2001:720:1710:11::2");
			return CONTENT;
		}
		return METHOD_NOT_ALLOWED;
	}else if(strcmp(request->uri_path,"/sensors/light")==0){
		PRINTF("Request Query to: /sensors/light \n");
		if (request->code==GET) {
			if(get_light_status()){				
			...
			if ((contain = strstr((char *)request->payload,"0")) != NULL) {
				...
				sprintf(response_buf, "You must indicate \"ON\" or \"OFF\" into payload.");
				return BAD_REQUEST; 
			}
			if(get_light_status()){				
				sprintf(response_buf, "ON");
