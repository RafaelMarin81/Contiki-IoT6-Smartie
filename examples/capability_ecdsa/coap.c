...
int parse_coap_packet(coap_packet_t *request, char *buf, int *need_confirm){
	...
	request->option_header_uri = buf[4];
	...
	int option_length;
	switch(request->option_header_uri&0xF0){
		case Option_Type_Uri_Path:
		    option_length = request->option_header_uri&0x0F;
		    //El valor de la opción está a 5 bytes del comienzo
			request->option_value_uri = buf + 5;
			request->option_value_uri [option_length] = 0;
			header_len += option_length;
	}
	header_len++;
	request->option_header_sig = buf[header_len];
	switch(request->option_header_sig&0xF0){
		case Option_Type_Sig:
		    option_length = request->option_header_sig&0x0F;
			request->option_value_sig = buf + header_len +1;
			request->option_value_sig [56] = 0;
			header_len += 57;
	}
	request->payload = buf+header_len;
	return request->code;
}
