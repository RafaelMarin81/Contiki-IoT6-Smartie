...
static void BlinkPRR(uint8_t PRR_value) {
    uint8_t i = 0;

    for(i = 0; i < (PRR_value/10); i++) {
            EnableDIO(E_AHI_DIO17_INT);   // Function to Switch-off the led.
            clock_delay(500);
            DisableDIO(E_AHI_DIO17_INT);   // Function to Switch-on the led.
            clock_delay(500);
    }
}
...
//        enviarInicio();     // button-RESET: Start the communication PAQ_INICIO.       Until a ACK is received.
            ...
            PRINTF("STATE_STOPING... PRR:%i\n", datalog_received_from_server.num_paq);
           
            BlinkPRR(datalog_received_from_server.num_paq);
...
//  bAHI_PhyRadioSetPower(3); // MAX_TX_POWER);      //Replace to vJPT_RadioSetPower(MAX_TX_POWER);
  ...
  uip_ip6addr(&ipaddr,0xfe80,0,0,0,0x0215,0x8d00,0x0000,0x0000);  // set_connection_address(&ipaddr);   for remote host.
