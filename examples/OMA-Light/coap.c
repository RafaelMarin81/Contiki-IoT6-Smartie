...
static void replace_gaps(char * payload,u8_t pos,u8_t len){
...
int create_coap_request(coap_packet_t *packet, message_type type, status_code_t code, char *payload){
	packet->header = COAP_VERSION | type;
	packet->code = code;
	packet->id = 0x0004;
	packet->payload = payload;
}
